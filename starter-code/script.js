    ...
    listItem.innerHTML = `
      ...
      item.name
        ...
        <button onclick="decreaseQuantity(${
          item.id
        }); decreaseCount();  calculateTOtalQuantity()"  class="item-button decrease">-</button>
        <span >${item.quantity}</span>
        <button onclick="increaseQuantity(${
          item.id
        }); increaseCount();  calculateTOtalQuantity()" class="item-button increase">+</button>
...
function getImageUrl(imageId) {
  ...
  // splits at "?" to get rid of query parameters,
  // and then checks if it matches with the home page.
  let imageMap;

  if (window.location.href.split("?")[0].endsWith("starter-code/")) {
    imageMap = {
...
function redirectToCheckout() {
  ...
  if (currentUrl.split("?")[0].endsWith("starter-code/")) {
